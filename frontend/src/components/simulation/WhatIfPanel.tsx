...
export default function WhatIfPanel() {
  const {
    whatIfParams, setWhatIfParams, profile, goals,
    simulationResults, setSimulationResults, setSimulationLoading,
  } = useAppStore()

  async function refreshSimulation(next: typeof whatIfParams) {
    if (!profile) return
    setSimulationLoading(true)
    try {
      const results = await runWhatIfScenario(profile, goals, next)
      setSimulationResults(results)
    } catch {
      setSimulationResults(runSimulation(profile, next, goals))
    } finally {
