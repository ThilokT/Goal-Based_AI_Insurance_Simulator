...
export default function LifeJourneyTimeline() {
  ...
  const {
    profile, goals, whatIfParams, setSimulationResults, simulationResults,
  ...
  useEffect(() => {
    ...
    runSimulationApi(profile, goals, whatIfParams)
      .then(results => {
        if (!cancelled) setSimulationResults(results)
      })
      .catch(() => {
        if (!cancelled) {
          setSimulationResults(runSimulation(profile, whatIfParams, goals))
        }
      })
      .finally(() => {
    ...
    <div className="space-y-4">
      <div className="card">
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="badge-orange text-xs">{simulationLoading ? 'Running…' : 'Live simulation'}</div>
        </div>
        <Legend />
