          ...
          <div className="p-5 flex flex-col">
            ...
            <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200 mb-4 flex justify-between items-center shadow-sm">
               <div>
                 ...
                 <p className="text-sm font-display font-bold text-gray-900">₹{formatExactRupee(overallReturns)}</p>
               </div>
               <div className="text-right">
                 <div className="bg-emerald-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                   {((globalWhatIfParams?.goalTargetAmounts && globalWhatIfParams.goalTargetAmounts[result?.goalId || goal?.id]) || goal?.corpusNeeded || 0) > 0 ? Math.round((overallReturns / ((globalWhatIfParams?.goalTargetAmounts && globalWhatIfParams.goalTargetAmounts[result?.goalId || goal?.id]) || goal?.corpusNeeded || 0)) * 100) : 0}% of Target
                 </div>
               </div>
            </div>
